...
function App() {
  ...
  const [subgenreFilter, setSubgenreFilter] = useState<HorrorSubgenre | null>(
    null
  );
  ...
  const favoriteBooks = filteredBooks.filter((book) =>
  ...
  function getPersonalizedRecommendations(): Book[] {
    ...
    const viewedSubgenres = currentHistory.flatMap(
      (entry) => entry.subgenres || []
    );
    const favoriteSubgenres = currentProfile.favoriteSubgenres;
    const allPreferredSubgenres = [
      ...new Set([...viewedSubgenres, ...favoriteSubgenres]),
    ];

    const scored = filteredBooks
      .filter(
        (book) => !currentHistory.some((entry) => entry.bookId === book.id)
      )
      .map((book) => {
        let score = 0;
